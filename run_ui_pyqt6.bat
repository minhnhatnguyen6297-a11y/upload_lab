...
set "SCRIPT_DIR=%~dp0"
set "BOOTSTRAP=%SCRIPT_DIR%bootstrap_ui_pyqt6.py"
...
echo [LOI] Khong bootstrap duoc Python 3.10+ cho giao dien moi.
set "EXIT_CODE=1"
goto :DONE

:TRY_FILE
if not exist "%~1" exit /b 1
"%~1" -c "import sys; raise SystemExit(0 if sys.version_info[:2] >= (3, 10) else 1)" >nul 2>&1
