...
REQUIRED_MODULES = ("docx", "dotenv", "playwright", "openpyxl", "PyQt6")
...
mods = {name: bool(util.find_spec(name)) for name in ("docx", "dotenv", "playwright", "openpyxl", "PyQt6")}
...
def recreate_venv(bootstrap_python: Path) -> None:
    if (BASE_DIR / ".venv").exists():
        ...
        log("[SETUP] Tao moi truong ao rieng cho giao dien moi (.venv)...")
...
def ensure_venv() -> Path:
    ...
    if not is_python_compatible(bootstrap_identity):
        ...
        raise RuntimeError(
            f"Python hien tai khong dat yeu cau >= {PYTHON_MIN_VERSION[0]}.{PYTHON_MIN_VERSION[1]}: {detail}"
        )
    ...
    if not VENV_PYTHON.exists():
        raise RuntimeError("Khong tao duoc .venv cho giao dien moi.")
    return VENV_PYTHON
...
def ensure_dependencies(python_exe: Path) -> None:
    ...
    if missing_modules:
        ...
        raise RuntimeError("Bootstrap giao dien moi chua san sang." + detail)
